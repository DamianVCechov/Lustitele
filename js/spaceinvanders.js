  ...
  for (var i = 0, len = aliens.length; i < len; i++) {
    ...
    if (alien.bullet !== null && checkRectCollision(alien.bullet.bounds, player.bounds)) {
      if (player.lives === 0) {
       ...
       particleManager.createExplosion(player.position.x, player.position.y, 'purple', 100, 8,8,6,0.001,40);
...
function drawStartScreen() {
  fillCenteredText("Space Invaders", CANVAS_WIDTH/2, CANVAS_HEIGHT/2.75, '#B302FF', 36);
  fillBlinkingText("Stiskni ENTER!", CANVAS_WIDTH/2, CANVAS_HEIGHT/2, 500, '#B302FF', 36);
}
